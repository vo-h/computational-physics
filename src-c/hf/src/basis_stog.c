...
double compute_nx(double A, double B, double alpha, double beta, int ai, int bi, double R, double t) {
    ...
    if (bi == 0) {
        double term1 = -(A-P);
        double term2 = -pow(t, 2) * (P-R) * compute_nx(A, B, alpha, beta, ai-1, 0, t, R);
        double term3 = (ai-1) / (2 * (alpha + beta)) * (1-pow(t,2)) * compute_nx(A, B, alpha, beta, ai-2, 0, t, R);
        return term1 + term2 + term3;
    }

    double term1 = compute_nx(A, B, alpha, beta, ai+1, bi-1, t, R);
    double term2 = (A-B)*compute_nx(A, B, alpha, beta, ai, bi-1, t, R);
    return term1 + term2;
}
